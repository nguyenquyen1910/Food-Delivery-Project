        ...
        if(isNaN(price)) {
            ...
            .then(data =>{
                if(data.success){
                    ...
                    if(data.exist){
                        toast({ title: "Error", message: "Sản phẩm đã tồn tại!", type: "error", duration: 3000 });
                    }
                    else{
                        toast({ title: "Error", message: "Thêm sản phẩm thất bại!", type: "error", duration: 3000 });
                    }
                }
            })
            .catch(error => {
                toast({ title: "Error", message: "Lỗi kết nối với máy chủ.", type: "error", duration: 3000 });
                console.log(error);
            });
        }
    }
