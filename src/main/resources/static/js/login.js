$(document).ready(function () {
  ...
  $("#login-button").on("click", function (event) {
    ...
    if (phonelog.length == 0) {
      $(".form-message.phonelog").text("Vui lòng nhập vào số điện thoại hoặc email");
    }
    ...
    else if(/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(phonelog)) {
      $(".form-message.phonelog").text("");
    }
    else{
      $(".form-message.phonelog").text("Vui lòng nhập email hợp lệ");
      $("#phone-login").val("");
    }
    ...
    if (passlog.length == 0) {
      ...
      $(".form-message-check-login").text("Vui lòng nhập mật khẩu lớn hơn 6 kí tự");
      $("#passwordlogin").val("");
    } else {
